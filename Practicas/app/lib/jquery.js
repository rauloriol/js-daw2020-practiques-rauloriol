...
(function (window, undefined) {
  ...
  jQuery.event = {
    ...
    trigger: function (event, data, elem, onlyHandlers) {
      ...
      let namespaces = event.namespace ? event.namespace.split('.') : [];
  ...
  (function (window, undefined) {
    ...
    function addCombinator(matcher, combinator, base) {
      ...
      const checkNonElements = base && combinator.dir === 'parentNode';
      const doneName = done++;
    ...
    function matcherFromGroupMatchers(elementMatchers, setMatchers) {
      ...
      const superMatcher = function (
        ...
        const dirrunsUnique = (dirruns += contextBackup == null ? 1 : Math.E);
  ...
  const rbracket = /\[\]$/;
  const rCRLF = /\r?\n/g;
  const rsubmitterTypes = /^(?:submit|button|image|reset)$/i;
